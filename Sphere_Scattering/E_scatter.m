...
% Set configuration
eps1 = 10;          % Permittivity of the sphere
mu1 = 1;            % Permeability of the sphere
r_a = 0.1;          % Radius of the sphere

fn = 0.5;           % Ceter frequency
f0 = 1e9;           % Frequency scale (f = fn * f0 Hz)
...
X = reshape(Es(:, 1), length(phi), length(theta));
Y = reshape(Es(:, 2), length(phi), length(theta));
Z = reshape(normEs, length(phi), length(theta));
...
% rr = 100 * ones(length(theta) * length(phi));
% [Xs, Ys, Zs] = sph2cart(Es(:, 2), tEs(:, 1), rr);
