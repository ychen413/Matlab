...
% Save Path
dir_ = 'D:\Research_USA\matlab_project\YagiUda\Data\dipole\reflector_corner\';
start = 1;
...
rcs_const = 5;              % Spacing between the plane and the antenna ( * lambda)
...
e = dipole;
rc = reflectorCorner;
corner_angle = 10:5:180;
tilt_angle = 90;

figC = figure;
t_start = tic;
for ind = start:length(corner_angle)
    % Exciter (dipole)
    ...
    rc.Spacing = rcs_const * lambda;
    rc.CornerAngle = corner_angle(ind);
    rc.Tilt = 0;
    rc.TiltAxis = [1 0 0];
    % show(rc)
    % view(0, 0)
    ...
    dir1 = strcat(dir_, 'alongX_', 'corner_', num2str(corner_angle(ind)), ...
                '_tilt_', num2str(tilt_angle), '_d_', num2str(rcs_const), 'lambda.dat');
